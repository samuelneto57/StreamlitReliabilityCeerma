...
def show():
    ...
    with st.expander("Acceleration Factor Information"):
        ...
        st.info('Given a model, AF can be calculated by stress levels:')
    ...
    if approach == options[0]:
        ...
        t_test = cols[1].number_input('Fixed accelerated test time',
                                      min_value=0.0, step=1.0, format='%0.2f')
    ...
    n_fail = st.number_input('Allowed number of failures in accelerated test',
                             min_value=0, step=1)
    params['allowed_failures'] = n_fail

    beta = st.number_input('Shape parameter of Weibull distribution',
